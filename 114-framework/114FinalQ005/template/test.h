 ...
 * ! I edit this
 */
#ifndef CATCH_CONFIG_DELIM
    #define CATCH_CONFIG_DELIM std::endl
#endif
...
constexpr const char* find_file_name(const char* str, const char* last_slash) {
    if (!*str) return last_slash;
    if (*str == '/' || *str == '\\') return find_file_name(str + 1, str + 1);
    return find_file_name(str + 1, last_slash);
}

constexpr const char* get_base_name(const char* str) {
    return find_file_name(str, str);
}

#define __FILENAME__ get_base_name(__FILE__)

#define CATCH_INTERNAL_LINEINFO \
    ::Catch::SourceLineInfo( __FILENAME__, static_cast<std::size_t>( __LINE__ ) )
 ...
 * ! I edit this
 */
#if defined(SECRET) && defined (ONLINE_JUDGE)
struct JudgeListener : public Catch::TestEventListenerBase {
  using TestEventListenerBase::TestEventListenerBase;

  void testRunStarting(Catch::TestRunInfo const& testRunInfo) override {
    std::printf("Valid: " SECRET "\n");
  }

  void testCaseEnded(Catch::TestCaseStats const& testCaseStats) override {
    auto newline = testCaseStats.totals.assertions.failed == 0 ? "" : "\n";
    std::printf("%sSuite: %s\n", newline, testCaseStats.testInfo.name.c_str());
  }

  void testRunEnded(Catch::TestRunStats const& testRunStats) override {
    auto caseName = testRunStats.runInfo.name.c_str();
    auto checkpointCount = testRunStats.totals.assertions.total();
    auto passed = testRunStats.totals.assertions.passed;
    std::printf("Has %ld Checkpoint, ", checkpointCount);

    if (passed == checkpointCount && checkpointCount > 0) {
      std::printf("PASSED");
    } else {
      std::printf("FAILED");
    }
...
CATCH_REGISTER_LISTENER(JudgeListener)
#endif
...
private:
    ...
    bool printInfoMessages;
        ...
        void CompactReporter::testRunEnded( TestRunStats const& _testRunStats ) {
             ...
             * ! I edit this
             */
            #ifdef CATCH_CONFIG_TOTAL_RESULT
